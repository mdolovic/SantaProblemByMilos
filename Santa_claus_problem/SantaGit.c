...
int main(int ac, char **av)
{
    ...
    for (int i = 0; i < MAX_REINDEERS; i++)
        reindeers[i] = CreateThread(Reindeer, (void *)(intptr_t)(i + 1));

    pthread_t *elves[MAX_ELVES];
    for (int j = 0; j < MAX_ELVES; j++)
    elves[j] = CreateThread(Elf, (void *)(intptr_t)(j + 1));
    ...
    for (int j = 0; j < MAX_REINDEERS; j++)
        pthread_cancel(*elves[j]);

    for (int i = 0; i < MAX_REINDEERS; i++)
        pthread_join(*reindeers[i], NULL);
    for (int j = 0; j < MAX_ELVES; j++)
        pthread_join(*elves[j], NULL);
